    ...
    string reorderSpaces(string text) {
        ...
        for (int i = 0; i < text.size();) {
            if (text[i] == ' ') {
                ...
                tokens.emplace_back(text.data() + begin, i - begin);
            }
        }

        // cout << space << tokens << endl;

        int space_gap = 0;
        int space_tail = space;

        if (space > 0 && tokens.size() > 1) {
            space_gap = space / (tokens.size() - 1);
            space_tail = space % (tokens.size() - 1);
        }
        ...
        for (int i = 0; i < tokens.size() - 1; i++) {
            res.append(tokens[i]);
            res.append(space_gap, ' ');
        }
        res.append(tokens.back());
        res.append(space_tail, ' ');
        return res;
    }
};
// @lc code=end
...
int main() {
    cout << Solution().reorderSpaces("a b c ") << "$" << endl;
    return 0;
}