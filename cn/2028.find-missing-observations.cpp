...
public:
    vector<int> missingRolls(vector<int>& rolls, int mean, int n) {
        int required = mean * (n + rolls.size());
        for (auto roll : rolls) required -= roll;

        vector<int> guess;

        if (required < n) return guess;
        if (required > 6 * n) return guess;

        int base_roll = required / n;
        int plus_num = required % n;
        for (int i = 0; i < n; i++) {
            if (plus_num-- > 0)
                guess.emplace_back(base_roll + 1);
            else
                guess.emplace_back(base_roll);
        }
...
int main() {
    ...
    cout << Solution().missingRolls(rolls, 3, 4) << endl;
    return 0;
}