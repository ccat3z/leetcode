...
public:
    ...
    int score(vector<int>& nums, int start, int end, int turn) {
        ...
        bool &h = hit[start][end];

        if (h) return c;
        if (start == end) return nums[start] * turn;

        int a = nums[start] * turn + score(nums, start+1, end, -turn);
        int b = nums[end] * turn + score(nums, start, end-1, -turn);

        h = true;
        c = max(a*turn, b*turn) * turn;
        return c;
    };
};
// @lc code=end
...
int main() {
    ...
    for (auto t : inputs) {
        cout << Solution().PredictTheWinner(t) << endl;
    }
    return 0;
}
