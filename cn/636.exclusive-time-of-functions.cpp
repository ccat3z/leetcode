...
public:
    vector<int> exclusiveTime(int n, vector<string>& logs) {
        ...
        int time;
        for (auto & log : logs) {
            parseLog(log, proc, type, time);

            switch (type) {
            case 's': 
                if (!sta.empty()) {
                    res[sta.top().first] += time - sta.top().second;
                }

                sta.emplace(proc, time);
                break;
            case 'e':
                res[proc] += time - sta.top().second + 1;
                sta.pop();
                if (!sta.empty()) {
                    sta.top().second = time + 1;
                }
                break;
            }
            // cout << res << endl;
    ...
    inline void parseLog(string &log, int &proc, char &type, int &time) {
        const char *data = log.c_str();

        // proc
        size_t len = 0;
        sscanf(data, "%d%n", &proc, &len);
        data += len;
