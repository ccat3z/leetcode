...
public:
    int *cache;
    int max_num;
    UnionFind(int max_num) : max_num(max_num) {
        cache = (int *) malloc((max_num+1) * sizeof(int));
        for (int i = 1; i <= max_num; ++i) cache[i] = i;
    }
    int find(int a) {
        int &c = cache[a];
        if (c != a) {
...
int main() {
    ...
    for (auto t : inputs) {
        cout << Solution().findRedundantConnection(t) << endl;
    }
}