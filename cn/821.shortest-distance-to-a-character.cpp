...
public:
    vector<int> shortestToChar(string s, char c) {
        ...
        for (int i = 0; i < s.size(); ++i) {
            ...
            res[i] = 0;
            int dest, j;
            for (dest = 1, j = i - 1; j >= 0 && res[j] > dest; --j, dest++) {
                res[j] = dest;
            }
            for (dest = 1, j = i + 1; j < s.size() && res[j] > dest; ++j, dest++) {
                res[j] = dest;
            }
        }
        return res;
...
int main() {
    cout << Solution().shortestToChar("loveleetcode", 'e') << endl;
    cout << Solution().shortestToChar("aaab", 'b') << endl;
    return 0;
}